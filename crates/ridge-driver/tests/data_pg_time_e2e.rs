//! End-to-end check that a Postgres `time` column round-trips a `Time`, against a
//! real database.
//!
//! Postgres delivers `time` (type OID 1083) as ISO `HH:MM:SS[.ffffff]` text over the
//! wire. The adapter decodes it into the typed `SqlTime` value instead of letting it
//! fall through to `SqlText`, and the codec parses that back into the `Time` ADT; a
//! bound `SqlTime` is sent as the same ISO text the database parses. So a time of day
//! survives the insert/select loop as the value it went in as. This is the decode path
//! the in-memory adapter cannot exercise (it has no OID wire form); `data_time_e2e`
//! covers the codec logic on the in-memory store.
//!
//! The program creates its own `ridge_pg_time` table with `Raw.exec`, so no
...
-- insert carries only the time and Postgres assigns the id 1.
pub type Item = { id: Int, atTime: Time } deriving (Row, Schema)
...
pub fn db setup () -> Result (Repo Item Postgres) Error =
    match connect (pgConfig ())
        Err e   -> Err e
        Ok conn ->
            let r: Repo Item Postgres = Repo.repo conn "ridge_pg_time"
            match Raw.exec conn "DROP TABLE IF EXISTS ridge_pg_time" []
                Err e -> Err e
                Ok _  ->
                    match Raw.exec conn "CREATE TABLE ridge_pg_time (id serial, at_time time)" []
                        Err e -> Err e
                        Ok _  ->
...
-- read the time back: a `SqlText` decode would leave it as a bare string, but the typed
-- `SqlTime` decode parses the `time` (OID 1083) into the structured value, recoverable
...
#[test]
fn postgres_time_round_trips_a_time() {
    if which::which("erlc").is_err() || which::which("erl").is_err() {
        eprintln!("erl/erlc not on PATH — skipping postgres_time_round_trips_a_time");
        return;
    }
    let url = match std::env::var("RIDGE_TEST_PG_URL") {
        Ok(u) => u,
        Err(_) => {
            eprintln!("RIDGE_TEST_PG_URL not set — skipping postgres_time_round_trips_a_time");
            return;
        }
    };
    ...
    let dir = tempfile::Builder::new()
        .prefix("ridge-pg-time-e2e-")
        .tempdir()
        .expect("temp dir");
    let cache = tempfile::Builder::new()
        .prefix("ridge-pg-time-e2e-cache-")
        .tempdir()
        .expect("cache dir");
    ...
    assert!(
        stdout.contains("roundTrip=13:45:30"),
        "missing `roundTrip=13:45:30` (a time column (OID 1083) decodes to the typed SqlTime and the time of day survives the round-trip through the database)\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
}
