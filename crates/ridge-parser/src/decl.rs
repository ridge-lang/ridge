...
/// Parse a `terminate` actor member: `terminate [caps] (params) = body`.
///
/// `terminate` is a contextual word (`LowerIdent`), exactly like `mailbox` —
/// no lexer keyword, so it stays a legal identifier elsewhere.
///
/// Precondition: `cur.peek()` is `Token::LowerIdent("terminate")`.
pub(crate) fn parse_terminate_decl(cur: &mut Cursor<'_>) -> Result<TerminateDecl, ParseError> {
    let start = cur.span();
    cur.bump(); // consume `terminate`
