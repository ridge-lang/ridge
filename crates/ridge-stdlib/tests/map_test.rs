...
fn assert_std_map_built() {
    ...
    assert!(
        summary.modules_built.iter().any(|m| m == "std.map"),
        "std.map must appear in modules_built; got: {:?}",
...
fn assert_map_ridge_discovered() {
    ...
    assert!(
        modules.iter().any(|m| m.name == "std.map"),
        "discover() must find std.map; found: {:?}",
