...
// `ManifestError` lives in `ridge-manifest`, the only manifest parser.
// This crate raises `M021`/`M022` from the resolve pass, which is the first
// point where the manifest and the module list are both in hand.
pub use ridge_manifest::ManifestError;
