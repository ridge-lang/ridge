//! End-to-end check that a Postgres `interval` column round-trips a `Duration`,
//! against a real database.
//!
//! Postgres delivers `interval` (type OID 1186) as its `postgres`-style text over the
//! wire. The adapter decodes that into the typed `SqlInterval` value (a whole-
//! millisecond span) instead of letting it fall through to `SqlText`, and the codec
...
pub fn db setup () -> Result (Repo Item Postgres) DbError =
    match connect (pgConfig ())
        Err e   -> Err e
        Ok conn ->
            let r: Repo Item Postgres = Repo.repo conn "ridge_pg_interval"
            let d = ofMillis 1500
...
-- millisecond span, recoverable as `1500` through the codec.
...
#[test]
fn postgres_interval_round_trips_a_duration() {
    if which::which("erlc").is_err() || which::which("erl").is_err() {
        eprintln!("erl/erlc not on PATH — skipping postgres_interval_round_trips_a_duration");
        return;
    }
    let Ok(url) = std::env::var("RIDGE_TEST_PG_URL") else {
        eprintln!("RIDGE_TEST_PG_URL not set — skipping postgres_interval_round_trips_a_duration");
        return;
    };
    ...
    let dir = tempfile::Builder::new()
        .prefix("ridge-pg-interval-e2e-")
        .tempdir()
        .expect("temp dir");
    let cache = tempfile::Builder::new()
        .prefix("ridge-pg-interval-e2e-cache-")
        .tempdir()
        .expect("cache dir");
