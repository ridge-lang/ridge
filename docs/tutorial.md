...
diagnostics in VS Code. Targets Ridge **0.2.9**.
...
```powershell
$env:RIDGE_VERSION = 'v0.2.9'
& ([scriptblock]::Create((iwr -useb 'https://raw.githubusercontent.com/ridge-lang/ridge/main/tools/install/install.ps1').Content))
```

```bash
RIDGE_VERSION=v0.2.9 bash -c "$(curl -fsSL https://raw.githubusercontent.com/ridge-lang/ridge/main/tools/install/install.sh)"
```
...
# expected: ridge 0.2.9
~/.cargo/bin/ridge-lsp --version
# expected: ridge-lsp 0.2.9
```
