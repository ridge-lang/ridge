//! Track-A tests for `std.fs` — 5 public functions.
//!
//! Each test asserts that `fs.ridge` compiles through the T4 build pipeline
...
fn assert_std_fs_built() {
    ...
    assert!(
        summary.modules_built.iter().any(|m| m == "std.fs"),
        "std.fs must appear in modules_built; got: {:?}",
...
fn assert_fs_ridge_discovered() {
    ...
    assert!(
        modules.iter().any(|m| m.name == "std.fs"),
        "discover() must find std.fs; found: {:?}",
