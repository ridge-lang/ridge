...
**Author:** Angel (creator) with design collaboration
...
#### Phase 3 Resolve questions (OQ-R001..OQ-R012)

Raised during Phase 3 (Resolve) planning. Each has a provisional answer that the implementation uses by default. If an OQ is challenged during Phase 3 implementation, revisit here; if accepted as-is through implementation, record as a new `D0NN` decision entry and move the row to §16.1.

| ID | Question | Impact | Provisional answer |
|----|----------|--------|--------------------|
| OQ-R001 ✅ | Bare `import foo.bar` (no `as`, no item list) — what does it expose? | Import-resolution semantics (§4.4 of resolve plan); affects lookup order and collision errors. | **Qualified-namespace only** (Elm-style `import Foo` — use-sites must write `Foo.bar`). Flooding the importer's scope with every `pub` symbol requires an explicit item list (`import foo.bar (a, b)`); wildcard `(..)` is reserved, not in 0.1.0. Rationale: avoids cross-import identifier collisions, keeps resolve-phase lookup order unambiguous, and aligns with Elm (Ridge's primary UX reference) rather than Rust's discouraged `use foo::*;` idiom. **Closed 2026-04-25: OQ accepted as-is; the R018 variant (bare-import-ambiguous) deleted (DR-07 / D073).** |
...
| OQ-R006 ✅ | How the Phase-3 stdlib manifest stays in sync with the eventual Phase-7 Ridge-written stdlib. | Long-term maintenance. | **Compile-time constant table in Phase 3; Phase 7 replaces with a generated table.** A regression test in Phase 7 re-parses every stdlib `.rg` file and asserts its `pub` exports match the constant table. Not a Phase 3 blocker. **Closed 2026-04-25 → D078.** |
...
| OQ-R008 ✅ | Should `NodeId` be promoted to a proper AST field in `ridge-ast` (micro-refactor) rather than the side-table `NodeIdMap`? | Upstream contract; re-snapshot Phase 2 tests. | **Defer.** Phase 3 uses the side-table `NodeIdMap`. Phase 4 may revisit when the type checker also wants stable IDs. _Known risk:_ a future Phase 5 transform that clones AST nodes must preserve `NodeId`s explicitly — the side-table approach has no type-system guarantee. Document this constraint in the `NodeIdMap` rustdoc when it lands so Phase 5 executors see it. **Closed 2026-04-25 → D079.** |
| OQ-R009 ✅ | `[dependencies]` table transitive conflicts between workspace members sharing a dep via `workspace-dependencies`. | Manifest resolution; §8.5 of resolve plan. | **Version pinning is the workspace's responsibility.** 0.1.0 has no semver solver (D023); each name resolves to exactly one entry. Conflicts across members emit `M013 UnknownWorkspaceMember` or `M015 WorkspaceDependencyAbsent`. **Closed 2026-04-25 → D080.** |
| OQ-R010 ✅ | Should the resolver produce per-module output when earlier parse errors produce a partial AST? | LSP UX. | **Yes.** Run resolution on the partial AST; skip `Expr`/`Pattern`/`Type` nodes that descend into error markers. Goal: LSP users get resolve diagnostics for the parts of a file that did parse, mirroring `rust-analyzer` / `tsc` behaviour. **Closed 2026-04-25 → D081.** |
...
| OQ-R012 ✅ | `spawn ActorName` — does the actor name need to be an unqualified `UPPER_IDENT`, or may it be a qualified path `Mod.ActorName`? | Spawn-expression grammar and resolve walker. | **Unqualified `Ident` only in 0.1.0** per grammar §6.19 (`"spawn" UPPER_IDENT { Expr }`). Cross-module spawn requires an `import` of the actor. Revisit in 0.2.0 if qualified-spawn ergonomics demand it. **Closed 2026-04-25 → D082.** |
| OQ-R013 ⚠ | Does Ridge have an implicit prelude that auto-imports `Option`/`Result` and their constructors (`Some`, `None`, `Ok`, `Err`) — and possibly `List` constructors — into every module? | Surfaces during Phase 3 T8 (intra-function scope walk): all 4 example programs (`log_analyzer.rg`, `url_shortener.rg`, `game_of_life.rg`, `rate_limiter.rg`) use `Some Info`, `None`, `Ok ()`, `return Err (...)` unqualified. With no prelude, every `.rg` file would need `import std.option (Some, None)` and `import std.result (Ok, Err)` boilerplate at the top. | **Provisional: implicit prelude.** Auto-import the following into every module's scope before per-module resolution: the `Option` type with constructors `Some` and `None`; the `Result` type with constructors `Ok` and `Err`. Matches Haskell's Prelude, Rust's `std::prelude::v1`, Elm's default imports. _Implementation:_ add a "prelude pass" before T7's per-module import resolution; it injects synthetic `EffectiveBinding` entries equivalent to `import std.option (Option, Some, None)` and `import std.result (Result, Ok, Err)` into every module's binding pool. Open question on scope creep: should the prelude also include `List`, `Map`, `Set` constructors? Conservative default = constructors of `Option` and `Result` only; revisit when concrete examples demand more. |
| OQ-R014 ⚠ | `let` followed by an indented continuation expression (`\|>` chain) inside a parenthesised lambda body fails to terminate at the next statement. The lexer's bracket-suppression of NEWLINE/INDENT/DEDENT (D070) folds subsequent statements into the let value, leaving downstream identifier uses unresolved. | Surfaces in `examples/game_of_life.rg` lines 59–64: a `let line = row \|> List.map ... \|> List.fold ... ""` followed by `Io.println $"\| ${line} \|"` parses with `${line}` unresolvable because the parser doesn't see a statement boundary inside the enclosing `()` — the parser walker stamps `Binding::Error` on the second-line use of `line`. | **Provisional: revisit D070.** Either (a) extend D070's bracket-suppression rule so a fresh statement at the same physical indent depth as the previous statement re-introduces a NEWLINE inside `()`/`[]`/`{}` brackets — i.e. layout suppression applies only to *line continuations* of the current expression, not to *new statements* at the same depth; or (b) require an explicit block delimiter (`do`-block, brace-delimited body, or named extracted function) for multi-statement lambda bodies. Option (a) is more ergonomic and keeps the `examples/game_of_life.rg` source unchanged; option (b) is simpler to implement but forces refactoring of the example. _Decision impact:_ option (a) is a Phase 2 (parser) reopening; option (b) is documentation only. Until resolved, the Phase 3 T8 acceptance test allowlists the `line` identifier — once D070 is settled, drop the allowlist entry. _Resolved 2026-04-24: option A — lexer emits NEWLINE inside brackets when col ≤ baseline; parser `parse_branch_body_flat` collects NEWLINE-separated statements as a flat `Expr::Block`. Drop `"line"` from T8 allowlist._ |
| OQ-R015 ⚠ | Should the implicit prelude also pre-bind `ModuleAlias` entries for pure-data stdlib modules (`Int`, `Float`, `Bool`, `Text`, `List`, `Map`, `Set`, `Json`) so that qualified names like `Int.parse`, `Text.padLeft`, `Float.fromInt`, `List.map` work without an explicit `import std.X as X` declaration? | Surfaces during Phase 3 T9 (qualified-name resolution): the 4 canonical examples use `Int.parse`, `Int.toText`, `Text.padLeft`, `Float.fromInt` without importing the corresponding stdlib modules. With no prelude alias these produce `R012 UnresolvedQualifiedName`. | **Resolved 2026-04-24: pre-bind module aliases for all pure-data stdlib modules; keep capability-bearing modules requiring explicit import.** Rationale: (1) preserves Ridge's capability-tracking principle — every side-effecting module remains visible at the import level; (2) matches ML/Haskell precedent where numeric and string primitives are available unqualified; (3) removes boilerplate from programs that use only data manipulation. _Pure-data modules added to prelude:_ `std.int` → `Int`, `std.float` → `Float`, `std.bool` → `Bool`, `std.text` → `Text`, `std.list` → `List`, `std.map` → `Map`, `std.set` → `Set`, `std.json` → `Json`. _Capability-bearing modules NOT in prelude (require explicit import):_ `std.io`, `std.fs`, `std.time`, `std.random`, `std.env`, `std.cli`, `std.proc`, `std.net.http`. _Implementation:_ third synthetic `ImportResolution` in `prelude_resolutions()` (`crates/ridge-resolve/src/imports.rs`) carrying 8 `ModuleAlias` `EffectiveBinding`s; user imports for the same `local_name` suppress the prelude binding (existing OQ-R013 suppression mechanism, which operates on `local_name` regardless of binding kind). `examples/url_shortener.rg` updated to add explicit `import std.net.http as Http` since `Http` is capability-bearing. `Error.text` in `log_analyzer.rg:91` remains an unresolved qualified name (separate issue — see recon note). |
| OQ-R016 ⚠ | Should the `ImportList` grammar accept `UPPER_IDENT` so users can write `import std.net.http (Request, Response, listen, respond) as Http` and reference the imported type / constructor names unqualified? Surfaces in `examples/url_shortener.rg` which uses `Response` and `Request` as bare identifiers in fn signatures and record constructors but only has `import std.net.http as Http` in scope. With the original `ImportList ::= LOWER_IDENT { "," LOWER_IDENT }` grammar, four `R010 UnresolvedIdent` errors fire on `Response` (spans 3123, 3206, 3466, 3571), and the only workarounds are (a) define the types locally (terrible DX, contradicts Ridge's DX-first principle) or (b) wait for qualified record-construction `Http.Response { ... }` support which requires AST changes (`Expr::Record::constructor: Ident → QualifiedName`) and is deferred to Phase 4. | Phase 3 T14 acceptance — `examples/url_shortener.rg` must resolve cleanly per phase DoD §14.4. Surfaces in 4 of 4 example programs as soon as any uses a stdlib type unqualified. Affects every future Ridge user of `std.net.http`, `std.json`, `std.fs` typed APIs, etc. | **Resolved 2026-04-25 → D072.** `ImportList` accepts both `LOWER_IDENT` and `UPPER_IDENT`. Rationale: aligns with Ridge's reference languages (Haskell, Elm, Rust all permit type/constructor imports in item lists). Implementation: 1-line grammar amendment (`grammar.ebnf:336`); ~5-line parser change (`crates/ridge-parser/src/decl.rs::parse_import` accepts `UpperIdent` in item-list arm); BUILTINS extension (`crates/ridge-resolve/src/stdlib_builtin.rs` — std.net.http exports gain `Request`, `Response`); zero AST or resolver changes (T7 lookup is case-agnostic and `ImportDecl::items: Vec<Ident>` already accepts both cases). `examples/url_shortener.rg` rewritten to `import std.net.http (Request, Response, listen, respond) as Http`. The complementary `Module.Type { ... }` qualified record-construction ergonomic remains a deferred Phase 4 feature — see §11.3 Phase 4 Tasks. |
