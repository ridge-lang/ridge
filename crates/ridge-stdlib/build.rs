// build.rs — Ridge stdlib build-script orchestrator (T4) + FFI-targets generator (T14.5.3).
//
...
// `include!`.  This is the canonical extractor.  Relocated from `crates/ridge-codegen-erl/build.rs` which
// held a per-consumer copy as a defensive cycle-break (T11/T11.5).  The cycle
// is confirmed absent: `ridge-codegen-erl → ridge-stdlib` introduces no
// cycle (D141).
//
    ...
    for (module, name) in ARITY_0_CONSTRUCTORS {
        ...
        match found {
            Some(entry) if entry.arity != 0 => {
                ...
                println!(
                    "cargo:warning=OQ-C034: constructor {module}::{name} has arity {arity} (expected 0)"
                );
                panic!(
            ...
            None => {
                println!(
                    "cargo:warning=OQ-C034: constructor {module}::{name} missing from FFI table"
                );
                panic!(
...
fn emit_rs(entries: &[FfiEntry]) -> String {
    ...
    // OQ-T14.5-03: OnceLock cache chosen over per-call clone (D141).

    let mut out = String::from("// @generated by crates/ridge-stdlib/build.rs (T14.5.3)\n");
    ...
    out.push_str("// single source of truth for path-B stdlib FFI targets (D141).\n");
    out.push_str("// T11.5: covers both @ffi stubs and pure-Ridge pub fn bodies.\n\n");
