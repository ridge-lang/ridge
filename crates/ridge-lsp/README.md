...
over **stdio transport only** (OQ-C001 — no `--tcp`, no named pipe, no Unix socket).
...
- **`textDocument/didSave`** — unconditional re-check of the workspace (OQ-C013).
- **`textDocument/didChange`** — debounced 250 ms (OQ-C013). Rapid edits within the
  debounce window collapse into one compile. In-flight compiles are cancelled when a
  new change arrives before the previous compile finishes (OQ-C016).
...
| `L802` | `LspMultiRootUnsupported` | Multi-root workspace → one-time warning; only the first root is used (OQ-C015) |
...
this means visible compile latency (> 10 s).  The in-flight cancellation (OQ-C016)
...
Deferred to T14 per §4 T14.  The VS Code extension scaffold that wires `ridge-lsp`
as its language server will be delivered in T14.
